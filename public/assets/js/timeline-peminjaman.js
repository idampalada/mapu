...
$(document).ready(function () {
  ...
  $("#formPengembalian").on("submit", function (event) {
    const isLateReturn =
      document.getElementById("is_late_return").value === "true";
    const alasanKeterlambatan = document.getElementById("alasan_keterlambatan");

    if (
      isLateReturn &&
      (!alasanKeterlambatan.value || alasanKeterlambatan.value.trim() === "")
    ) {
      event.preventDefault();
      alasanKeterlambatan.classList.add("is-invalid");
      alasanKeterlambatan.focus();
      return false;
    }

    alasanKeterlambatan.classList.remove("is-invalid");
  });

  // Kamera untuk dokumentasi keterlambatan
  let streamLate = null;

  // Tombol untuk membuka kamera
  $("#btn-camera-late-doc").on("click", function () {
    openCameraLate();
  });

  // Tombol untuk mengambil foto
  $("#btn-take-photo-late").on("click", function () {
    takePhotoLate();
  });

  // Tombol untuk menutup kamera
  $("#btn-cancel-camera-late").on("click", function () {
    closeCameraLate();
  });

  // Tombol untuk mengambil ulang foto
  $("#btn-retake-photo-late").on("click", function () {
    $("#photo-preview-late").addClass("d-none");
    openCameraLate();
  });

  // Tombol untuk menggunakan foto
  $("#btn-use-photo-late").on("click", function () {
    usePhotoLate();
  });

  // Function untuk membuka kamera
  function openCameraLate() {
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
      navigator.mediaDevices
        .getUserMedia({ video: { facingMode: "environment" } })
        .then(function (stream) {
          streamLate = stream;
          document.getElementById("camera-feed-late").srcObject = stream;
          $("#camera-container-late").removeClass("d-none");
        })
  ...
  function closeCameraLate() {
    if (streamLate) {
      streamLate.getTracks().forEach((track) => track.stop());
      streamLate = null;
    }
    $("#camera-container-late").addClass("d-none");
  }

  // Function untuk mengambil foto
  function takePhotoLate() {
    if (!streamLate) return;

    const video = document.getElementById("camera-feed-late");
    const canvas = document.getElementById("photo-canvas-late");
    ...
    // Sembunyikan kamera dan tampilkan preview
    $("#camera-container-late").addClass("d-none");
    $("#photo-preview-late").removeClass("d-none");
  }

  // Function untuk menggunakan foto yang diambil
  function usePhotoLate() {
    const canvas = document.getElementById("photo-canvas-late");
    ...
    canvas.toBlob(
      function (blob) {
        // Create file object
        const fileName = "keterlambatan_" + new Date().getTime() + ".jpg";
        ...
        try {
          ...
          document.getElementById("foto_keterlambatan").files =
            dataTransfer.files;
          ...
          alert("Foto dokumentasi keterlambatan berhasil disimpan");
        } catch (e) {
