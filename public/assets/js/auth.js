      ...
      password: {
        ...
        rules: {
          ...
          pattern: {
            ...
            message:
              "Kata Sandi harus mengandung minimal 6 karakter, karakter khusus dan angka.",
          },
        },
      },
      pass_confirm: {
  ...
  async submitForm() {
    try {
      ...
      if (data.success) {
        ...
        document
          .querySelector("#resetSuccessModal .btn-primary")
        ...
        if (data.errors) {
          ...
          document.getElementById("resetFailedMessage").textContent =
            data.message;
      ...
      password: {
        ...
        rules: {
          ...
          pattern: {
            ...
            message:
              "Kata Sandi harus mengandung huruf besar, huruf kecil, angka, dan karakter.",
          },
        },
      },
      pass_confirm: {
  ...
  async submitForm() {
    try {
      ...
      if (data.success) {
        ...
        document
          .querySelector("#registerSuccessModal .btn-primary")
          .addEventListener("click", () => {
            window.location.href = "/login";
          });
      } else {
...
class UserManagement {
  ...
  constructor() {
    ...
    if (this.unitOrganisasiSelect && this.unitKerjaSelect) {
      this.unitOrganisasiSelect.addEventListener("change", () => {
        this.updateUnitKerjaOptions();
      });
      this.updateUnitKerjaOptions();
    }

    this.initializeValidation(modalAddUser);
    this.initializeFormSubmit(modalAddUser);

    modalAddUser.addEventListener("hidden.bs.modal", () => {
      const form = modalAddUser.querySelector("form");
      if (form) {
        form.reset();
  ...
  updateUnitKerjaOptions() {
    if (!this.unitKerjaSelect || !this.unitOrganisasiSelect) return;

    const selectedOrganisasi = this.unitOrganisasiSelect.value;
    this.unitKerjaSelect.innerHTML = "";

    const placeholderOption = document.createElement("option");
    placeholderOption.value = "";
    placeholderOption.selected = true;
    placeholderOption.disabled = true;
    this.unitKerjaSelect.appendChild(placeholderOption);

    if (
      selectedOrganisasi &&
      UserManagement.unitKerjaOptions[selectedOrganisasi]
    ) {
      UserManagement.unitKerjaOptions[selectedOrganisasi].forEach((kerja) => {
        const option = document.createElement("option");
        option.value = kerja;
        option.textContent = kerja;
        this.unitKerjaSelect.appendChild(option);
      });
    }
  }

  initializeValidation(modalAddUser) {
    const form = modalAddUser.querySelector("#registerForm");
    if (!form) return;
    ...
    inputs.forEach((input) => {
      const debouncedValidate = debounce(() => this.validateField(input), 500);
    ...
    selects.forEach((select) => {
      ...
      select.addEventListener("blur", () => this.validateField(select));
    });
  }
  ...
  async validateField(field) {
    ...
    switch (name) {
      case "fullname":
        if (!value) {
          return this.showError(field, "Nama Lengkap harus diisi");
        } else if (value.length < 4) {
          return this.showError(field, "Nama Lengkap minimal 4 karakter");
        }
        break;
      case "username":
        if (!value) {
          return this.showError(field, "Username harus diisi");
        } else if (value.length < 4) {
          return this.showError(field, "Username minimal 4 karakter");
      ...
      case "email":
        if (!value) {
          return this.showError(field, "Email harus diisi");
      ...
      case "password":
        if (!value) {
          return this.showError(field, "Kata Sandi harus diisi");
        } else if (
      ...
      case "pass_confirm":
        ...
        if (!value) {
          return this.showError(field, "Konfirmasi Kata Sandi harus diisi");
        } else if (value !== password) {
          return this.showError(field, "Konfirmasi Kata Sandi tidak cocok");
        }
        break;
    }
  ...
  resetValidation(form) {
    ...
    fields.forEach((field) => this.resetFieldValidation(field));
  }
  ...
  initializeFormSubmit(modalAddUser) {
    const form = modalAddUser.querySelector("#registerForm");
    if (!form) return;
        ...
        success: (response) => {
          if (response.success) {
            ...
            Swal.fire({
              icon: "success",
              title: "Berhasil!",
              text: response.message || "Pengguna baru berhasil ditambahkan",
              showConfirmButton: false,
              timer: 1500,
            }).then(() => {
              location.reload();
