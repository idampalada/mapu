...
<head>
    ...
    <title>Kategori Alat Pertanian</title>
...
<body>
    <div class="container">
        ...
        <div class="mb-3 text-end">
            ...
            <form action="<?= base_url('user/barang/peralatandanmesin/alatpertanian/importFromApi') ?>" method="post" class="d-inline">
                <button type="submit" class="btn btn-success" onclick="return confirm('Import/sync data dari API Alat Pertanian? (Data existing akan di-update, data baru akan ditambahkan)')">
                    <i class="bi bi-cloud-download"></i> Import/Sync API
                </button>
            </form>
            
            <!-- Tombol Reset Data -->
            <form action="<?= base_url('user/barang/peralatandanmesin/alatpertanian/resetData') ?>" method="post" class="d-inline">
                <button type="submit" class="btn btn-danger" onclick="return confirm('PERINGATAN: Ini akan menghapus SEMUA data alat pertanian! Yakin ingin melanjutkan?')">
        ...
        <div class="alert alert-info mb-3">
            ...
            <ul class="mb-0">
                <li><strong>Import/Sync API:</strong> Mengambil data dari API Peralatan dan Mesin Non-TIK kategori 3.04 (Alat Pertanian)</li>
        ...
        <!-- Kategori Alat Pertanian (1 Kategori) -->
        <div class="grid mb-4">
            <?php
                $buttons = [
                    'ALAT PENGOLAHAN' => 'bi-gear-wide-connected'
                ];
            ...
            ?>
            <a href="<?= base_url('user/barang/peralatandanmesin/alatpertanian/kelompokalatpertanian/' . $urlLabel) ?>" class="item <?= $isActive ? 'active' : '' ?>">
        ...
        <div class="d-flex justify-content-between mb-3">
            ...
            <div class="dropdown">
                ...
                <ul class="dropdown-menu" aria-labelledby="sortDropdown">
                    <li><a class="dropdown-item <?= (isset($sort) && $sort == 'kode_barang' && isset($order) && $order == 'asc') ? 'active' : '' ?>" href="<?= base_url('user/barang/peralatandanmesin/alatpertanian/kelompokalatpertanian/' . urlencode($active) . '?sort=kode_barang&order=asc') ?>">
                        <i class="fas fa-sort-alpha-down"></i> Kode Barang (A-Z)
                    </a></li>
                    <li><a class="dropdown-item <?= (isset($sort) && $sort == 'kode_barang' && isset($order) && $order == 'desc') ? 'active' : '' ?>" href="<?= base_url('user/barang/peralatandanmesin/alatpertanian/kelompokalatpertanian/' . urlencode($active) . '?sort=kode_barang&order=desc') ?>">
                        <i class="fas fa-sort-alpha-up-alt"></i> Kode Barang (Z-A)
                    </a></li>
                    <li><hr class="dropdown-divider"></li>
                    <li><a class="dropdown-item <?= (isset($sort) && $sort == 'nama_barang' && isset($order) && $order == 'asc') ? 'active' : '' ?>" href="<?= base_url('user/barang/peralatandanmesin/alatpertanian/kelompokalatpertanian/' . urlencode($active) . '?sort=nama_barang&order=asc') ?>">
                        <i class="fas fa-sort-alpha-down"></i> Nama Barang (A-Z)
                    </a></li>
                    <li><a class="dropdown-item <?= (isset($sort) && $sort == 'nama_barang' && isset($order) && $order == 'desc') ? 'active' : '' ?>" href="<?= base_url('user/barang/peralatandanmesin/alatpertanian/kelompokalatpertanian/' . urlencode($active) . '?sort=nama_barang&order=desc') ?>">
                        <i class="fas fa-sort-alpha-up-alt"></i> Nama Barang (Z-A)
                    </a></li>
                    <li><hr class="dropdown-divider"></li>
                    <li><a class="dropdown-item <?= (isset($sort) && $sort == 'nilai_perolehan' && isset($order) && $order == 'asc') ? 'active' : '' ?>" href="<?= base_url('user/barang/peralatandanmesin/alatpertanian/kelompokalatpertanian/' . urlencode($active) . '?sort=nilai_perolehan&order=asc') ?>">
                        <i class="fas fa-sort-numeric-down"></i> Nilai Perolehan (Terendah)
                    </a></li>
                    <li><a class="dropdown-item <?= (isset($sort) && $sort == 'nilai_perolehan' && isset($order) && $order == 'desc') ? 'active' : '' ?>" href="<?= base_url('user/barang/peralatandanmesin/alatpertanian/kelompokalatpertanian/' . urlencode($active) . '?sort=nilai_perolehan&order=desc') ?>">
            ...
            <div>
                <?php if ($active === 'ALAT PENGOLAHAN'): ?>
                    <a href="<?= base_url('user/barang/peralatandanmesin/alatpertanian/exportAlatPertanianList/alatpengolahan') ?>" class="btn btn-success">Ekspor Data Alat Pengolahan</a>
                <?php endif; ?>
            </div>
        </div>
        ...
        <!-- Form Tambah Alat Pertanian -->
        <div class="card mb-4" id="formTambahAlatPertanian" style="display: none;">
            <div class="card-header bg-primary text-white">
                <strong>Form Tambah <?= esc($activeKelompok) ?></strong>
            </div>
            <div class="card-body">
                <form action="<?= base_url('user/barang/peralatandanmesin/alatpertanian/tambah') ?>" method="post">
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="kelompok" class="form-select" required>
                                ...
                                <option value="ALAT PENGOLAHAN" <?= ($activeKelompok === 'ALAT PENGOLAHAN' || old('kelompok') === 'ALAT PENGOLAHAN') ? 'selected' : '' ?>>Alat Pengolahan</option>
                            </select>
                        </div>
                        <div class="col-md-6 mb-3">
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <textarea name="spek_lain" class="form-control" rows="2" placeholder="Deskripsi spesifikasi tambahan alat pertanian"><?= old('spek_lain') ?></textarea>
                        </div>
                        <div class="col-md-12 mb-3">
                            <label for="status_penggunaan" class="form-label">Status Penggunaan</label>
                            <textarea name="status_penggunaan" class="form-control" rows="2" placeholder="Digunakan untuk keperluan pertanian"><?= old('status_penggunaan') ?></textarea>
                        </div>
                    </div>
        ...
        <?php if (!empty($alatPertanianList)): ?>
        <div class="table-responsive mt-5">
            <table class="table table-bordered table-hover align-middle">
                ...
                <tbody>
                    <?php $no = 1; foreach ($alatPertanianList as $item): ?>
                        <tr>
        ...
        <div class="d-flex justify-content-between align-items-center mt-4">
            <div>
                <p class="mb-0">Menampilkan <?= count($alatPertanianList) ?> dari <?= isset($totalItems) ? esc($totalItems) : count($alatPertanianList) ?> data</p>
            </div>
            <div>
                    ...
                    <nav aria-label="Page navigation">
                        <ul class="pagination">
                            <?php if (isset($currentPage) && $currentPage > 1): ?>
                                <li class="page-item">
                                    <a class="page-link" href="<?= base_url('user/barang/peralatandanmesin/alatpertanian/kelompokalatpertanian/' . urlencode($activeKelompok)) ?>?page=1<?= !empty($searchTerm) ? '&search=' . urlencode($searchTerm) : '' ?><?= !empty($sort) && !empty($order) ? '&sort=' . urlencode($sort) . '&order=' . urlencode($order) : '' ?>" aria-label="First">
                                        <span aria-hidden="true">&laquo;&laquo;</span>
                                    </a>
                                </li>
                                <li class="page-item">
                                    <a class="page-link" href="<?= base_url('user/barang/peralatandanmesin/alatpertanian/kelompokalatpertanian/' . urlencode($activeKelompok)) ?>?page=<?= $currentPage - 1 ?><?= !empty($searchTerm) ? '&search=' . urlencode($searchTerm) : '' ?><?= !empty($sort) && !empty($order) ? '&sort=' . urlencode($sort) . '&order=' . urlencode($order) : '' ?>" aria-label="Previous">
                            ...
                            for ($i = $start; $i <= $end; $i++): ?>
                                <li class="page-item <?= isset($currentPage) && $i == $currentPage ? 'active' : '' ?>">
                                    <a class="page-link" href="<?= base_url('user/barang/peralatandanmesin/alatpertanian/kelompokalatpertanian/' . urlencode($activeKelompok)) ?>?page=<?= $i ?><?= !empty($searchTerm) ? '&search=' . urlencode($searchTerm) : '' ?><?= !empty($sort) && !empty($order) ? '&sort=' . urlencode($sort) . '&order=' . urlencode($order) : '' ?>">
                                        <?= $i ?>
                                    </a>
                                </li>
                            <?php endfor; ?>
                            ...
                            <?php if (isset($currentPage) && isset($totalPages) && $currentPage < $totalPages): ?>
                                <li class="page-item">
                                    <a class="page-link" href="<?= base_url('user/barang/peralatandanmesin/alatpertanian/kelompokalatpertanian/' . urlencode($activeKelompok)) ?>?page=<?= $currentPage + 1 ?><?= !empty($searchTerm) ? '&search=' . urlencode($searchTerm) : '' ?><?= !empty($sort) && !empty($order) ? '&sort=' . urlencode($sort) . '&order=' . urlencode($order) : '' ?>" aria-label="Next">
                                        <span aria-hidden="true">&raquo;</span>
                                    </a>
                                </li>
                                <li class="page-item">
                                    <a class="page-link" href="<?= base_url('user/barang/peralatandanmesin/alatpertanian/kelompokalatpertanian/' . urlencode($activeKelompok)) ?>?page=<?= $totalPages ?><?= !empty($searchTerm) ? '&search=' . urlencode($searchTerm) : '' ?><?= !empty($sort) && !empty($order) ? '&sort=' . urlencode($sort) . '&order=' . urlencode($order) : '' ?>" aria-label="Last">
        ...
        <?php else: ?>
            <div class="alert alert-info text-center">
                ...
                <p>Silakan gunakan tombol "Import/Sync API" untuk mengimpor data dari API Peralatan dan Mesin Non-TIK kategori 3.04 ke database.</p>
            </div>
        <?php endif; ?>
    ...
    document.getElementById('toggleFormBtn').addEventListener('click', function () {
        const form = document.getElementById('formTambahAlatPertanian');
