...
<head>
    ...
    <title>Kategori Alat Peraga</title>
...
<body>
    <div class="container">
        ...
        <div class="mb-3 text-end">
            ...
            <form action="<?= base_url('user/barang/peralatandanmesin/alatperaga/importFromApi') ?>" method="post" class="d-inline">
                <button type="submit" class="btn btn-success" onclick="return confirm('Import/sync data dari API Alat Peraga? (Data existing akan di-update, data baru akan ditambahkan)')">
                    <i class="bi bi-cloud-download"></i> Import/Sync API
                </button>
            </form>
            
            <!-- Tombol Reset Data -->
            <form action="<?= base_url('user/barang/peralatandanmesin/alatperaga/resetData') ?>" method="post" class="d-inline">
                <button type="submit" class="btn btn-danger" onclick="return confirm('PERINGATAN: Ini akan menghapus SEMUA data alat peraga! Yakin ingin melanjutkan?')">
        ...
        <div class="alert alert-info mb-3">
            ...
            <ul class="mb-0">
                <li><strong>Import/Sync API:</strong> Mengambil data dari API Peralatan dan Mesin Non-TIK kategori 3.16 (Alat Peraga)</li>
        ...
        <!-- Kategori Alat Peraga (1 Kategori) -->
        <div class="grid mb-4">
            <?php
                $buttons = [
                    'ALAT PERAGA PELATIHAN DAN PERCONTOHAN' => 'bi-easel'
                ];
            ...
            ?>
            <a href="<?= base_url('user/barang/peralatandanmesin/alatperaga/kelompokalatperaga/' . $urlLabel) ?>" class="item <?= $isActive ? 'active' : '' ?>">
        ...
        <div class="d-flex justify-content-between mb-3">
            ...
            <div class="dropdown">
                ...
                <ul class="dropdown-menu" aria-labelledby="sortDropdown">
                    <li><a class="dropdown-item <?= (isset($sort) && $sort == 'kode_barang' && isset($order) && $order == 'asc') ? 'active' : '' ?>" href="<?= base_url('user/barang/peralatandanmesin/alatperaga/kelompokalatperaga/' . urlencode($active) . '?sort=kode_barang&order=asc') ?>">
                        <i class="fas fa-sort-alpha-down"></i> Kode Barang (A-Z)
                    </a></li>
                    <li><a class="dropdown-item <?= (isset($sort) && $sort == 'kode_barang' && isset($order) && $order == 'desc') ? 'active' : '' ?>" href="<?= base_url('user/barang/peralatandanmesin/alatperaga/kelompokalatperaga/' . urlencode($active) . '?sort=kode_barang&order=desc') ?>">
                        <i class="fas fa-sort-alpha-up-alt"></i> Kode Barang (Z-A)
                    </a></li>
                    <li><hr class="dropdown-divider"></li>
                    <li><a class="dropdown-item <?= (isset($sort) && $sort == 'nama_barang' && isset($order) && $order == 'asc') ? 'active' : '' ?>" href="<?= base_url('user/barang/peralatandanmesin/alatperaga/kelompokalatperaga/' . urlencode($active) . '?sort=nama_barang&order=asc') ?>">
                        <i class="fas fa-sort-alpha-down"></i> Nama Barang (A-Z)
                    </a></li>
                    <li><a class="dropdown-item <?= (isset($sort) && $sort == 'nama_barang' && isset($order) && $order == 'desc') ? 'active' : '' ?>" href="<?= base_url('user/barang/peralatandanmesin/alatperaga/kelompokalatperaga/' . urlencode($active) . '?sort=nama_barang&order=desc') ?>">
                        <i class="fas fa-sort-alpha-up-alt"></i> Nama Barang (Z-A)
                    </a></li>
                    <li><hr class="dropdown-divider"></li>
                    <li><a class="dropdown-item <?= (isset($sort) && $sort == 'nilai_perolehan' && isset($order) && $order == 'asc') ? 'active' : '' ?>" href="<?= base_url('user/barang/peralatandanmesin/alatperaga/kelompokalatperaga/' . urlencode($active) . '?sort=nilai_perolehan&order=asc') ?>">
                        <i class="fas fa-sort-numeric-down"></i> Nilai Perolehan (Terendah)
                    </a></li>
                    <li><a class="dropdown-item <?= (isset($sort) && $sort == 'nilai_perolehan' && isset($order) && $order == 'desc') ? 'active' : '' ?>" href="<?= base_url('user/barang/peralatandanmesin/alatperaga/kelompokalatperaga/' . urlencode($active) . '?sort=nilai_perolehan&order=desc') ?>">
            ...
            <div>
                <?php if ($active === 'ALAT PERAGA PELATIHAN DAN PERCONTOHAN'): ?>
                    <a href="<?= base_url('user/barang/peralatandanmesin/alatperaga/exportAlatPeragaList/alatperagapelatihandanpercontohan') ?>" class="btn btn-success">Ekspor Data Alat Peraga</a>
                <?php endif; ?>
            </div>
        </div>
        ...
        <!-- Form Tambah Alat Peraga -->
        <div class="card mb-4" id="formTambahAlatPeraga" style="display: none;">
            <div class="card-header bg-primary text-white">
                <strong>Form Tambah <?= esc($activeKelompok) ?></strong>
            </div>
            <div class="card-body">
                <form action="<?= base_url('user/barang/peralatandanmesin/alatperaga/tambah') ?>" method="post">
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="kelompok" class="form-select" required>
                                ...
                                <option value="ALAT PERAGA PELATIHAN DAN PERCONTOHAN" <?= ($activeKelompok === 'ALAT PERAGA PELATIHAN DAN PERCONTOHAN' || old('kelompok') === 'ALAT PERAGA PELATIHAN DAN PERCONTOHAN') ? 'selected' : '' ?>>Alat Peraga Pelatihan dan Percontohan</option>
                            </select>
                        </div>
                        <div class="col-md-6 mb-3">
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <textarea name="spek_lain" class="form-control" rows="2" placeholder="Deskripsi spesifikasi tambahan alat peraga"><?= old('spek_lain') ?></textarea>
                        </div>
                        <div class="col-md-12 mb-3">
                            <label for="status_penggunaan" class="form-label">Status Penggunaan</label>
                            <textarea name="status_penggunaan" class="form-control" rows="2" placeholder="Digunakan untuk keperluan pelatihan dan percontohan"><?= old('status_penggunaan') ?></textarea>
                        </div>
                    </div>
        ...
        <div class="table-responsive mt-5">
            <table class="table table-bordered table-hover align-middle">
                ...
                <tbody>
                    <?php $no = 1; foreach ($alatPeragaList as $item): ?>
                        <tr>
        ...
        <div class="d-flex justify-content-between align-items-center mt-4">
            <div>
                <p class="mb-0">Menampilkan <?= count($alatPeragaList) ?> dari <?= isset($totalItems) ? esc($totalItems) : count($alatPeragaList) ?> data</p>
            </div>
            <div>
                    ...
                    <nav aria-label="Page navigation">
                        <ul class="pagination">
                            <?php if (isset($currentPage) && $currentPage > 1): ?>
                                <li class="page-item">
                                    <a class="page-link" href="<?= base_url('user/barang/peralatandanmesin/alatperaga/kelompokalatperaga/' . urlencode($activeKelompok)) ?>?page=1<?= !empty($searchTerm) ? '&search=' . urlencode($searchTerm) : '' ?><?= !empty($sort) && !empty($order) ? '&sort=' . urlencode($sort) . '&order=' . urlencode($order) : '' ?>" aria-label="First">
                                        <span aria-hidden="true">&laquo;&laquo;</span>
                                    </a>
                                </li>
                                <li class="page-item">
                                    <a class="page-link" href="<?= base_url('user/barang/peralatandanmesin/alatperaga/kelompokalatperaga/' . urlencode($activeKelompok)) ?>?page=<?= $currentPage - 1 ?><?= !empty($searchTerm) ? '&search=' . urlencode($searchTerm) : '' ?><?= !empty($sort) && !empty($order) ? '&sort=' . urlencode($sort) . '&order=' . urlencode($order) : '' ?>" aria-label="Previous">
                            ...
                            for ($i = $start; $i <= $end; $i++): ?>
                                <li class="page-item <?= isset($currentPage) && $i == $currentPage ? 'active' : '' ?>">
                                    <a class="page-link" href="<?= base_url('user/barang/peralatandanmesin/alatperaga/kelompokalatperaga/' . urlencode($activeKelompok)) ?>?page=<?= $i ?><?= !empty($searchTerm) ? '&search=' . urlencode($searchTerm) : '' ?><?= !empty($sort) && !empty($order) ? '&sort=' . urlencode($sort) . '&order=' . urlencode($order) : '' ?>">
                                        <?= $i ?>
                                    </a>
                                </li>
                            <?php endfor; ?>
                            ...
                            <?php if (isset($currentPage) && isset($totalPages) && $currentPage < $totalPages): ?>
                                <li class="page-item">
                                    <a class="page-link" href="<?= base_url('user/barang/peralatandanmesin/alatperaga/kelompokalatperaga/' . urlencode($activeKelompok)) ?>?page=<?= $currentPage + 1 ?><?= !empty($searchTerm) ? '&search=' . urlencode($searchTerm) : '' ?><?= !empty($sort) && !empty($order) ? '&sort=' . urlencode($sort) . '&order=' . urlencode($order) : '' ?>" aria-label="Next">
                                        <span aria-hidden="true">&raquo;</span>
                                    </a>
                                </li>
                                <li class="page-item">
                                    <a class="page-link" href="<?= base_url('user/barang/peralatandanmesin/alatperaga/kelompokalatperaga/' . urlencode($activeKelompok)) ?>?page=<?= $totalPages ?><?= !empty($searchTerm) ? '&search=' . urlencode($searchTerm) : '' ?><?= !empty($sort) && !empty($order) ? '&sort=' . urlencode($sort) . '&order=' . urlencode($order) : '' ?>" aria-label="Last">
        ...
        <?php else: ?>
            <div class="alert alert-info text-center">
                ...
                <p>Silakan gunakan tombol "Import/Sync API" untuk mengimpor data dari API Peralatan dan Mesin Non-TIK kategori 3.16 ke database.</p>
            </div>
        <?php endif; ?>
    ...
    document.getElementById('toggleFormBtn').addEventListener('click', function () {
        const form = document.getElementById('formTambahAlatPeraga');
