...
use App\Models\AlatProduksiPengolahanModel;

class AlatProduksiPengolahan extends BaseController
{
    protected $alatProduksiPengolahanModel;
    
    public function __construct()
    {
        $this->alatProduksiPengolahanModel = new AlatProduksiPengolahanModel();
    }
    ...
    {
        $alatProduksiPengolahanList = $this->getApiData();
        return view('user/barang/peralatandanmesin/alatproduksipengolahan/dashboard', [
            'alatProduksiPengolahanList' => $alatProduksiPengolahanList
        ]);
    }

    public function kelompokAlatProduksiPengolahan()
    {
        $sort = $this->request->getGet('sort') ?? 'kode_barang';
        $order = $this->request->getGet('order') ?? 'asc';
        
        $allAlatProduksiPengolahanList = $this->alatProduksiPengolahanModel->findAll();
        
        // Filter data berdasarkan kelompok - 3 kategori
        $kelompokList = [
            'SUMUR',
            'PRODUKSI',
        ...
        foreach ($kelompokList as $kelompok) {
            $data = array_filter($allAlatProduksiPengolahanList, function ($item) use ($kelompok) {
        ...
        return view('user/barang/peralatandanmesin/alatproduksipengolahan/kelompokalatproduksipengolahan', [
            'sort' => $sort,
            'order' => $order,
            'allData' => $allData,
    ...
    public function kelompokDetail($kelompok)
    {
        ...
        if (!$this->alatProduksiPengolahanModel->isValidKelompokAlatProduksiPengolahan($kelompok)) {
        ...
        $alatProduksiPengolahanList = $this->alatProduksiPengolahanModel->searchAlatProduksiPengolahan(
            $searchTerm, 
            $kelompok, 
            $perPage, 
            ($page - 1) * $perPage
        ...
        if (empty($alatProduksiPengolahanList) && !empty($searchTerm)) {
            $alatProduksiPengolahanList = $this->alatProduksiPengolahanModel->searchAlatProduksiPengolahan(
                $searchTerm, 
                '', 
                $perPage, 
        ...
        if (!empty($alatProduksiPengolahanList)) {
            usort($alatProduksiPengolahanList, function($a, $b) use ($sort, $order) {
                $aVal = $a[$sort] ?? '';
                $bVal = $b[$sort] ?? '';
        ...
        $totalItems = !empty($searchTerm) 
            ? count($this->alatProduksiPengolahanModel->searchAlatProduksiPengolahan($searchTerm, $kelompok, 999999, 0))
            : $this->alatProduksiPengolahanModel->countByKelompok($kelompok);

        $pager = service('pager');
        $pager->setPath('user/barang/peralatandanmesin/alatproduksipengolahan/kelompokalatproduksipengolahan/' . 
                        urlencode($kelompok));
    ...
    // Method untuk menambah alat produksi pengolahan manual
    public function tambah()
    {
        log_message('info', '=== TAMBAH ALAT PRODUKSI PENGOLAHAN METHOD DIPANGGIL ===');
        ...
        if ($isPost && (!empty($postData) || !empty($postRaw))) {
            ...
            $kategori_detail = $this->alatProduksiPengolahanModel->mapKelompokToKategori($kelompok);
            ...
            $data = [
                ...
                'kategori_utama' => 'ALAT PRODUKSI PENGOLAHAN DAN PEMURNIAN',
            ...
            try {
                $this->alatProduksiPengolahanModel->skipValidation(true);
                $insertResult = $this->alatProduksiPengolahanModel->insert($data);
                
                if ($insertResult) {
                    $insertId = $this->alatProduksiPengolahanModel->getInsertID();
                    session()->setFlashdata('success', "Data alat produksi pengolahan berhasil disimpan! ID: {$insertId}");
                } else {
                    $errors = $this->alatProduksiPengolahanModel->errors();
                    session()->setFlashdata('error', 'Gagal menyimpan data: ' . implode(', ', $errors));
                }
                
                $this->alatProduksiPengolahanModel->skipValidation(false);
    ...
    {
        log_message('info', '=== IMPORT ALAT PRODUKSI PENGOLAHAN FROM API STARTED ===');
        ...
        try {
            ...
            // Kelompok yang valid untuk kategori 3.13
            $validKelompok = [
                'SUMUR',
                'PRODUKSI',
                'PENGOLAHAN DAN PEMURNIAN'
            ...
            foreach ($allApiData as $index => $apiItem) {
                try {
                    ...
                    // FILTER: Hanya import data dengan kelompok kategori 3.13 yang valid
                    if (!in_array($kelompok_raw, $validKelompok)) {
                        $filtered++;
                        log_message('info', "Filtered out: {$kode_barang_original} - Kelompok: '{$kelompok_raw}' (bukan kategori 3.13)");
                        continue;
                    }
                    ...
                    $cleanData = $this->alatProduksiPengolahanModel->cleanImportData($apiItem);
                    ...
                    // Cek kelompok valid
                    if (!$this->alatProduksiPengolahanModel->isValidKelompokAlatProduksiPengolahan($cleanData['kelompok'])) {
                    ...
                    $existingData = $this->alatProduksiPengolahanModel->findByKodeBarang($unique_kode);
                    
                    if ($existingData) {
                        // Update existing data
                        $result = $this->alatProduksiPengolahanModel->updateData($existingData['id'], $cleanData);
                        if ($result) {
                            $updated++;
                        ...
                        // Insert new data
                        $result = $this->alatProduksiPengolahanModel->insertData($cleanData);
                        if ($result) {
                            $imported++;
    ...
    {
        try {
            $this->alatProduksiPengolahanModel->builder()->truncate();
    ...
    public function exportAlatProduksiPengolahanList($kelompokSlug)
    {
        // Mapping slug ke kelompok yang sebenarnya
        $kelompokMapping = [
            'sumur' => 'SUMUR',
            'produksi' => 'PRODUKSI',
            'pengolahandanpemurnian' => 'PENGOLAHAN DAN PEMURNIAN'
        ];
        ...
        try {
            $data = $this->alatProduksiPengolahanModel->getByKelompok($kelompok);
            ...
            $filename = 'alat_produksi_pengolahan_' . strtolower(str_replace([' ', '/'], '_', $kelompok)) . '_' . date('Y-m-d_H-i-s') . '.csv';
    ...
    {
        try {
            $stats = $this->alatProduksiPengolahanModel->getDashboardData();
    ...
    public function testApi()
    {
        try {
            ...
            echo "<h3>Test API Alat Produksi Pengolahan</h3>";
            ...
            if (!empty($apiData)) {
                ...
                echo "<h4>Data yang akan diimport (kategori 3.13):</h4>";
                echo "<p>Total: " . count($filteredData) . " dari " . count($apiData) . " data</p>";
                
                if (!empty($filteredData)) {
                    echo "<h5>Sample data kategori 3.13:</h5>";
    ...
    public function search()
    {
        ...
        try {
            $results = $this->alatProduksiPengolahanModel->searchAlatProduksiPengolahan($searchTerm, $kelompok, 10);
