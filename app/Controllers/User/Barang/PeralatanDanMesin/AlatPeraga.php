...
use App\Models\AlatPeragaModel;

class AlatPeraga extends BaseController
{
    protected $alatPeragaModel;
    
    public function __construct()
    {
        $this->alatPeragaModel = new AlatPeragaModel();
    }
    ...
    {
        $alatPeragaList = $this->getApiData();
        return view('user/barang/peralatandanmesin/alatperaga/dashboard', [
            'alatPeragaList' => $alatPeragaList
        ]);
    }

    public function kelompokAlatPeraga()
    {
        $sort = $this->request->getGet('sort') ?? 'kode_barang';
        $order = $this->request->getGet('order') ?? 'asc';
        
        $allAlatPeragaList = $this->alatPeragaModel->findAll();
        ...
        foreach ($kelompokList as $kelompok) {
            $data = array_filter($allAlatPeragaList, function ($item) use ($kelompok) {
        ...
        return view('user/barang/peralatandanmesin/alatperaga/kelompokalatperaga', [
            'sort' => $sort,
            'order' => $order,
            'allData' => $allData,
    ...
    public function kelompokDetail($kelompok)
    {
        ...
        if (!$this->alatPeragaModel->isValidKelompokAlatPeraga($kelompok)) {
        ...
        $alatPeragaList = $this->alatPeragaModel->searchAlatPeraga(
            $searchTerm, 
            $kelompok, 
            $perPage, 
            ($page - 1) * $perPage
        ...
        if (empty($alatPeragaList) && !empty($searchTerm)) {
            $alatPeragaList = $this->alatPeragaModel->searchAlatPeraga(
                $searchTerm, 
                '', 
                $perPage, 
        ...
        if (!empty($alatPeragaList)) {
            usort($alatPeragaList, function($a, $b) use ($sort, $order) {
                $aVal = $a[$sort] ?? '';
                $bVal = $b[$sort] ?? '';
        ...
        $totalItems = !empty($searchTerm) 
            ? count($this->alatPeragaModel->searchAlatPeraga($searchTerm, $kelompok, 999999, 0))
            : $this->alatPeragaModel->countByKelompok($kelompok);

        $pager = service('pager');
        $pager->setPath('user/barang/peralatandanmesin/alatperaga/kelompokalatperaga/' . 
                        urlencode($kelompok));
    ...
    // Method untuk menambah alat peraga manual
    public function tambah()
    {
        log_message('info', '=== TAMBAH ALAT PERAGA METHOD DIPANGGIL ===');
        ...
        if ($isPost && (!empty($postData) || !empty($postRaw))) {
            ...
            $kategori_detail = $this->alatPeragaModel->mapKelompokToKategori($kelompok);
            ...
            try {
                $this->alatPeragaModel->skipValidation(true);
                $insertResult = $this->alatPeragaModel->insert($data);
                
                if ($insertResult) {
                    $insertId = $this->alatPeragaModel->getInsertID();
                    session()->setFlashdata('success', "Data alat peraga berhasil disimpan! ID: {$insertId}");
                } else {
                    $errors = $this->alatPeragaModel->errors();
                    session()->setFlashdata('error', 'Gagal menyimpan data: ' . implode(', ', $errors));
                }
                
                $this->alatPeragaModel->skipValidation(false);
    ...
    {
        log_message('info', '=== IMPORT ALAT PERAGA FROM API STARTED ===');
        ...
        try {
            ...
            // Kelompok yang valid untuk kategori 3.16
            $validKelompok = [
                'ALAT PERAGA PELATIHAN DAN PERCONTOHAN'
            ];
            ...
            foreach ($allApiData as $index => $apiItem) {
                try {
                    ...
                    // FILTER: Hanya import data dengan kelompok kategori 3.16 yang valid
                    if (!in_array($kelompok_raw, $validKelompok)) {
                        $filtered++;
                        log_message('info', "Filtered out: {$kode_barang_original} - Kelompok: '{$kelompok_raw}' (bukan kategori 3.16)");
                        continue;
                    }
                    ...
                    $cleanData = $this->alatPeragaModel->cleanImportData($apiItem);
                    ...
                    // Cek kelompok valid
                    if (!$this->alatPeragaModel->isValidKelompokAlatPeraga($cleanData['kelompok'])) {
                    ...
                    $existingData = $this->alatPeragaModel->findByKodeBarang($unique_kode);
                    
                    if ($existingData) {
                        // Update existing data
                        $result = $this->alatPeragaModel->updateData($existingData['id'], $cleanData);
                        if ($result) {
                            $updated++;
                        ...
                        // Insert new data
                        $result = $this->alatPeragaModel->insertData($cleanData);
                        if ($result) {
                            $imported++;
    ...
    {
        try {
            $this->alatPeragaModel->builder()->truncate();
    ...
    public function exportAlatPeragaList($kelompokSlug)
    {
        // Mapping slug ke kelompok yang sebenarnya
        $kelompokMapping = [
            'alatperagapelatihandanpercontohan' => 'ALAT PERAGA PELATIHAN DAN PERCONTOHAN'
        ];
        ...
        try {
            $data = $this->alatPeragaModel->getByKelompok($kelompok);
            ...
            $filename = 'alat_peraga_' . strtolower(str_replace([' ', '/', '&', '-'], '_', $kelompok)) . '_' . date('Y-m-d_H-i-s') . '.csv';
    ...
    {
        try {
            $stats = $this->alatPeragaModel->getDashboardData();
    ...
    public function testApi()
    {
        try {
            ...
            echo "<h3>Test API Alat Peraga</h3>";
            ...
            if (!empty($apiData)) {
                ...
                echo "<h4>Data yang akan diimport (kategori 3.16):</h4>";
                echo "<p>Total: " . count($filteredData) . " dari " . count($apiData) . " data</p>";
                
                if (!empty($filteredData)) {
                    echo "<h5>Sample data kategori 3.16:</h5>";
    ...
    public function search()
    {
        ...
        try {
            $results = $this->alatPeragaModel->searchAlatPeraga($searchTerm, $kelompok, 10);
