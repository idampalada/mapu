...
use App\Models\AlatPersenjataanModel;

class AlatPersenjataan extends BaseController
{
    protected $alatPersenjataanModel;
    
    public function __construct()
    {
        $this->alatPersenjataanModel = new AlatPersenjataanModel();
    }
    ...
    {
        $alatPersenjataanList = $this->getApiData();
        return view('user/barang/peralatandanmesin/alatpersenjataan/dashboard', [
            'alatPersenjataanList' => $alatPersenjataanList
        ]);
    }

    public function kelompokAlatPersenjataan()
    {
        $sort = $this->request->getGet('sort') ?? 'kode_barang';
        $order = $this->request->getGet('order') ?? 'asc';
        
        $allAlatPersenjataanList = $this->alatPersenjataanModel->findAll();
        ...
        foreach ($kelompokList as $kelompok) {
            $data = array_filter($allAlatPersenjataanList, function ($item) use ($kelompok) {
        ...
        return view('user/barang/peralatandanmesin/alatpersenjataan/kelompokalatpersenjataan', [
            'sort' => $sort,
            'order' => $order,
            'allData' => $allData,
    ...
    public function kelompokDetail($kelompok)
    {
        ...
        if (!$this->alatPersenjataanModel->isValidKelompokAlatPersenjataan($kelompok)) {
        ...
        $alatPersenjataanList = $this->alatPersenjataanModel->searchAlatPersenjataan(
            $searchTerm, 
            $kelompok, 
            $perPage, 
            ($page - 1) * $perPage
        ...
        if (empty($alatPersenjataanList) && !empty($searchTerm)) {
            $alatPersenjataanList = $this->alatPersenjataanModel->searchAlatPersenjataan(
                $searchTerm, 
                '', 
                $perPage, 
        ...
        if (!empty($alatPersenjataanList)) {
            usort($alatPersenjataanList, function($a, $b) use ($sort, $order) {
                $aVal = $a[$sort] ?? '';
                $bVal = $b[$sort] ?? '';
        ...
        $totalItems = !empty($searchTerm) 
            ? count($this->alatPersenjataanModel->searchAlatPersenjataan($searchTerm, $kelompok, 999999, 0))
            : $this->alatPersenjataanModel->countByKelompok($kelompok);

        $pager = service('pager');
        $pager->setPath('user/barang/peralatandanmesin/alatpersenjataan/kelompokalatpersenjataan/' . 
                        urlencode($kelompok));
    ...
    // Method untuk menambah alat persenjataan manual
    public function tambah()
    {
        log_message('info', '=== TAMBAH ALAT PERSENJATAAN METHOD DIPANGGIL ===');
        ...
        if ($isPost && (!empty($postData) || !empty($postRaw))) {
            ...
            $kategori_detail = $this->alatPersenjataanModel->mapKelompokToKategori($kelompok);
            ...
            $data = [
                ...
                'kategori_utama' => 'ALAT PERSENJATAAN',
            ...
            try {
                $this->alatPersenjataanModel->skipValidation(true);
                $insertResult = $this->alatPersenjataanModel->insert($data);
                
                if ($insertResult) {
                    $insertId = $this->alatPersenjataanModel->getInsertID();
                    session()->setFlashdata('success', "Data alat persenjataan berhasil disimpan! ID: {$insertId}");
                } else {
                    $errors = $this->alatPersenjataanModel->errors();
                    session()->setFlashdata('error', 'Gagal menyimpan data: ' . implode(', ', $errors));
                }
                
                $this->alatPersenjataanModel->skipValidation(false);
    ...
    {
        log_message('info', '=== IMPORT ALAT PERSENJATAAN FROM API STARTED ===');
        ...
        try {
            ...
            // Kelompok yang valid untuk kategori 3.09
            $validKelompok = [
                'SENJATA API',
                'PERSENJATAAN NON SENJATA API',
                'SENJATA SINAR',
            ...
            foreach ($allApiData as $index => $apiItem) {
                try {
                    ...
                    // FILTER: Hanya import data dengan kelompok kategori 3.09 yang valid
                    if (!in_array($kelompok_raw, $validKelompok)) {
                        $filtered++;
                        log_message('info', "Filtered out: {$kode_barang_original} - Kelompok: '{$kelompok_raw}' (bukan kategori 3.09)");
                        continue;
                    }
                    ...
                    $cleanData = $this->alatPersenjataanModel->cleanImportData($apiItem);
                    ...
                    // Cek kelompok valid
                    if (!$this->alatPersenjataanModel->isValidKelompokAlatPersenjataan($cleanData['kelompok'])) {
                    ...
                    $existingData = $this->alatPersenjataanModel->findByKodeBarang($unique_kode);
                    
                    if ($existingData) {
                        // Update existing data
                        $result = $this->alatPersenjataanModel->updateData($existingData['id'], $cleanData);
                        if ($result) {
                            $updated++;
                        ...
                        // Insert new data
                        $result = $this->alatPersenjataanModel->insertData($cleanData);
                        if ($result) {
                            $imported++;
    ...
    {
        try {
            $this->alatPersenjataanModel->builder()->truncate();
    ...
    public function exportAlatPersenjataanList($kelompokSlug)
    {
        // Mapping slug ke kelompok yang sebenarnya
        $kelompokMapping = [
            'senjataapi' => 'SENJATA API',
            'persenjataannonsenjataapi' => 'PERSENJATAAN NON SENJATA API',
            'senjatasinar' => 'SENJATA SINAR',
            'alatkhususkepolisian' => 'ALAT KHUSUS KEPOLISIAN'
        ];
        ...
        try {
            $data = $this->alatPersenjataanModel->getByKelompok($kelompok);
            ...
            $filename = 'alat_persenjataan_' . strtolower(str_replace([' ', '/'], '_', $kelompok)) . '_' . date('Y-m-d_H-i-s') . '.csv';
    ...
    {
        try {
            $stats = $this->alatPersenjataanModel->getDashboardData();
    ...
    public function testApi()
    {
        try {
            ...
            echo "<h3>Test API Alat Persenjataan</h3>";
            ...
            if (!empty($apiData)) {
                ...
                echo "<h4>Data yang akan diimport (kategori 3.09):</h4>";
                echo "<p>Total: " . count($filteredData) . " dari " . count($apiData) . " data</p>";
                
                if (!empty($filteredData)) {
                    echo "<h5>Sample data kategori 3.09:</h5>";
    ...
    public function search()
    {
        ...
        try {
            $results = $this->alatPersenjataanModel->searchAlatPersenjataan($searchTerm, $kelompok, 10);
