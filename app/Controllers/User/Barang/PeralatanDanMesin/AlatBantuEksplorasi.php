...
use App\Models\AlatBantuEksplorasiModel;

class AlatBantuEksplorasi extends BaseController
{
    protected $alatBantuEksplorasiModel;
    
    public function __construct()
    {
        $this->alatBantuEksplorasiModel = new AlatBantuEksplorasiModel();
    }
    ...
    {
        $alatBantuEksplorasiList = $this->getApiData();
        return view('user/barang/peralatandanmesin/alatbantueksplorasi/dashboard', [
            'alatBantuEksplorasiList' => $alatBantuEksplorasiList
        ]);
    }

    public function kelompokAlatBantuEksplorasi()
    {
        $sort = $this->request->getGet('sort') ?? 'kode_barang';
        $order = $this->request->getGet('order') ?? 'asc';
        
        $allAlatBantuEksplorasiList = $this->alatBantuEksplorasiModel->findAll();
        ...
        foreach ($kelompokList as $kelompok) {
            $data = array_filter($allAlatBantuEksplorasiList, function ($item) use ($kelompok) {
        ...
        return view('user/barang/peralatandanmesin/alatbantueksplorasi/kelompokalatbantueksplorasi', [
            'sort' => $sort,
            'order' => $order,
            'allData' => $allData,
    ...
    public function kelompokDetail($kelompok)
    {
        ...
        if (!$this->alatBantuEksplorasiModel->isValidKelompokAlatBantuEksplorasi($kelompok)) {
        ...
        $alatBantuEksplorasiList = $this->alatBantuEksplorasiModel->searchAlatBantuEksplorasi(
            $searchTerm, 
            $kelompok, 
            $perPage, 
            ($page - 1) * $perPage
        ...
        if (empty($alatBantuEksplorasiList) && !empty($searchTerm)) {
            $alatBantuEksplorasiList = $this->alatBantuEksplorasiModel->searchAlatBantuEksplorasi(
                $searchTerm, 
                '', 
                $perPage, 
        ...
        if (!empty($alatBantuEksplorasiList)) {
            usort($alatBantuEksplorasiList, function($a, $b) use ($sort, $order) {
                $aVal = $a[$sort] ?? '';
                $bVal = $b[$sort] ?? '';
        ...
        $totalItems = !empty($searchTerm) 
            ? count($this->alatBantuEksplorasiModel->searchAlatBantuEksplorasi($searchTerm, $kelompok, 999999, 0))
            : $this->alatBantuEksplorasiModel->countByKelompok($kelompok);

        $pager = service('pager');
        $pager->setPath('user/barang/peralatandanmesin/alatbantueksplorasi/kelompokalatbantueksplorasi/' . 
                        urlencode($kelompok));
    ...
    // Method untuk menambah alat bantu eksplorasi manual
    public function tambah()
    {
        log_message('info', '=== TAMBAH ALAT BANTU EKSPLORASI METHOD DIPANGGIL ===');
        ...
        if ($isPost && (!empty($postData) || !empty($postRaw))) {
            ...
            $kategori_detail = $this->alatBantuEksplorasiModel->mapKelompokToKategori($kelompok);
            ...
            $data = [
                ...
                'kategori_utama' => 'ALAT BANTU EKSPLORASI',
            ...
            try {
                $this->alatBantuEksplorasiModel->skipValidation(true);
                $insertResult = $this->alatBantuEksplorasiModel->insert($data);
                
                if ($insertResult) {
                    $insertId = $this->alatBantuEksplorasiModel->getInsertID();
                    session()->setFlashdata('success', "Data alat bantu eksplorasi berhasil disimpan! ID: {$insertId}");
                } else {
                    $errors = $this->alatBantuEksplorasiModel->errors();
                    session()->setFlashdata('error', 'Gagal menyimpan data: ' . implode(', ', $errors));
                }
                
                $this->alatBantuEksplorasiModel->skipValidation(false);
    ...
    {
        log_message('info', '=== IMPORT ALAT BANTU EKSPLORASI FROM API STARTED ===');
        ...
        try {
            ...
            // Kelompok yang valid untuk kategori 3.14
            $validKelompok = [
                'ALAT BANTU EKSPLORASI',
                'ALAT BANTU PRODUKSI'
            ];
            ...
            foreach ($allApiData as $index => $apiItem) {
                try {
                    ...
                    // FILTER: Hanya import data dengan kelompok kategori 3.14 yang valid
                    if (!in_array($kelompok_raw, $validKelompok)) {
                        $filtered++;
                        log_message('info', "Filtered out: {$kode_barang_original} - Kelompok: '{$kelompok_raw}' (bukan kategori 3.14)");
                        continue;
                    }
                    ...
                    $cleanData = $this->alatBantuEksplorasiModel->cleanImportData($apiItem);
                    ...
                    // Cek kelompok valid
                    if (!$this->alatBantuEksplorasiModel->isValidKelompokAlatBantuEksplorasi($cleanData['kelompok'])) {
                    ...
                    $existingData = $this->alatBantuEksplorasiModel->findByKodeBarang($unique_kode);
                    
                    if ($existingData) {
                        // Update existing data
                        $result = $this->alatBantuEksplorasiModel->updateData($existingData['id'], $cleanData);
                        if ($result) {
                            $updated++;
                        ...
                        // Insert new data
                        $result = $this->alatBantuEksplorasiModel->insertData($cleanData);
                        if ($result) {
                            $imported++;
    ...
    {
        try {
            $this->alatBantuEksplorasiModel->builder()->truncate();
    ...
    public function exportAlatBantuEksplorasiList($kelompokSlug)
    {
        // Mapping slug ke kelompok yang sebenarnya
        $kelompokMapping = [
            'alatbantueksplorasi' => 'ALAT BANTU EKSPLORASI',
            'alatbantuproduksi' => 'ALAT BANTU PRODUKSI'
        ];
        ...
        try {
            $data = $this->alatBantuEksplorasiModel->getByKelompok($kelompok);
            ...
            $filename = 'alat_bantu_eksplorasi_' . strtolower(str_replace([' ', '/'], '_', $kelompok)) . '_' . date('Y-m-d_H-i-s') . '.csv';
    ...
    {
        try {
            $stats = $this->alatBantuEksplorasiModel->getDashboardData();
    ...
    public function testApi()
    {
        try {
            ...
            echo "<h3>Test API Alat Bantu Eksplorasi</h3>";
            ...
            if (!empty($apiData)) {
                ...
                echo "<h4>Data yang akan diimport (kategori 3.14):</h4>";
                echo "<p>Total: " . count($filteredData) . " dari " . count($apiData) . " data</p>";
                
                if (!empty($filteredData)) {
                    echo "<h5>Sample data kategori 3.14:</h5>";
    ...
    public function search()
    {
        ...
        try {
            $results = $this->alatBantuEksplorasiModel->searchAlatBantuEksplorasi($searchTerm, $kelompok, 10);
