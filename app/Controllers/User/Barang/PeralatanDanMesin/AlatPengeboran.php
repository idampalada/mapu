...
use App\Models\AlatPengeboranModel;

class AlatPengeboran extends BaseController
{
    protected $alatPengeboranModel;
    
    public function __construct()
    {
        $this->alatPengeboranModel = new AlatPengeboranModel();
    }
    ...
    {
        $alatPengeboranList = $this->getApiData();
        return view('user/barang/peralatandanmesin/alatpengeboran/dashboard', [
            'alatPengeboranList' => $alatPengeboranList
        ]);
    }

    public function kelompokAlatPengeboran()
    {
        $sort = $this->request->getGet('sort') ?? 'kode_barang';
        $order = $this->request->getGet('order') ?? 'asc';
        
        $allAlatPengeboranList = $this->alatPengeboranModel->findAll();
        ...
        foreach ($kelompokList as $kelompok) {
            $data = array_filter($allAlatPengeboranList, function ($item) use ($kelompok) {
        ...
        return view('user/barang/peralatandanmesin/alatpengeboran/kelompokalatpengeboran', [
            'sort' => $sort,
            'order' => $order,
            'allData' => $allData,
    ...
    public function kelompokDetail($kelompok)
    {
        ...
        if (!$this->alatPengeboranModel->isValidKelompokAlatPengeboran($kelompok)) {
        ...
        $alatPengeboranList = $this->alatPengeboranModel->searchAlatPengeboran(
            $searchTerm, 
            $kelompok, 
            $perPage, 
            ($page - 1) * $perPage
        ...
        if (empty($alatPengeboranList) && !empty($searchTerm)) {
            $alatPengeboranList = $this->alatPengeboranModel->searchAlatPengeboran(
                $searchTerm, 
                '', 
                $perPage, 
        ...
        if (!empty($alatPengeboranList)) {
            usort($alatPengeboranList, function($a, $b) use ($sort, $order) {
                $aVal = $a[$sort] ?? '';
                $bVal = $b[$sort] ?? '';
        ...
        $totalItems = !empty($searchTerm) 
            ? count($this->alatPengeboranModel->searchAlatPengeboran($searchTerm, $kelompok, 999999, 0))
            : $this->alatPengeboranModel->countByKelompok($kelompok);

        $pager = service('pager');
        $pager->setPath('user/barang/peralatandanmesin/alatpengeboran/kelompokalatpengeboran/' . 
                        urlencode($kelompok));
    ...
    // Method untuk menambah alat pengeboran manual
    public function tambah()
    {
        log_message('info', '=== TAMBAH ALAT PENGEBORAN METHOD DIPANGGIL ===');
        ...
        if ($isPost && (!empty($postData) || !empty($postRaw))) {
            ...
            $kategori_detail = $this->alatPengeboranModel->mapKelompokToKategori($kelompok);
            ...
            $data = [
                ...
                'kategori_utama' => 'ALAT PENGEBORAN',
            ...
            try {
                $this->alatPengeboranModel->skipValidation(true);
                $insertResult = $this->alatPengeboranModel->insert($data);
                
                if ($insertResult) {
                    $insertId = $this->alatPengeboranModel->getInsertID();
                    session()->setFlashdata('success', "Data alat pengeboran berhasil disimpan! ID: {$insertId}");
                } else {
                    $errors = $this->alatPengeboranModel->errors();
                    session()->setFlashdata('error', 'Gagal menyimpan data: ' . implode(', ', $errors));
                }
                
                $this->alatPengeboranModel->skipValidation(false);
    ...
    {
        log_message('info', '=== IMPORT ALAT PENGEBORAN FROM API STARTED ===');
        ...
        try {
            ...
            // Kelompok yang valid untuk kategori 3.12
            $validKelompok = [
                'ALAT PENGEBORAN MESIN',
                'ALAT PENGEBORAN NON MESIN'
            ];
            ...
            foreach ($allApiData as $index => $apiItem) {
                try {
                    ...
                    // FILTER: Hanya import data dengan kelompok kategori 3.12 yang valid
                    if (!in_array($kelompok_raw, $validKelompok)) {
                        $filtered++;
                        log_message('info', "Filtered out: {$kode_barang_original} - Kelompok: '{$kelompok_raw}' (bukan kategori 3.12)");
                        continue;
                    }
                    ...
                    $cleanData = $this->alatPengeboranModel->cleanImportData($apiItem);
                    ...
                    // Cek kelompok valid
                    if (!$this->alatPengeboranModel->isValidKelompokAlatPengeboran($cleanData['kelompok'])) {
                    ...
                    $existingData = $this->alatPengeboranModel->findByKodeBarang($unique_kode);
                    
                    if ($existingData) {
                        // Update existing data
                        $result = $this->alatPengeboranModel->updateData($existingData['id'], $cleanData);
                        if ($result) {
                            $updated++;
                        ...
                        // Insert new data
                        $result = $this->alatPengeboranModel->insertData($cleanData);
                        if ($result) {
                            $imported++;
    ...
    {
        try {
            $this->alatPengeboranModel->builder()->truncate();
    ...
    public function exportAlatPengeboranList($kelompokSlug)
    {
        // Mapping slug ke kelompok yang sebenarnya
        $kelompokMapping = [
            'alatpengeboranmesin' => 'ALAT PENGEBORAN MESIN',
            'alatpengeborannonmesin' => 'ALAT PENGEBORAN NON MESIN'
        ];
        ...
        try {
            $data = $this->alatPengeboranModel->getByKelompok($kelompok);
            ...
            $filename = 'alat_pengeboran_' . strtolower(str_replace([' ', '/', '&', '-'], '_', $kelompok)) . '_' . date('Y-m-d_H-i-s') . '.csv';
    ...
    {
        try {
            $stats = $this->alatPengeboranModel->getDashboardData();
    ...
    public function testApi()
    {
        try {
            ...
            echo "<h3>Test API Alat Pengeboran</h3>";
            ...
            if (!empty($apiData)) {
                ...
                echo "<h4>Data yang akan diimport (kategori 3.12):</h4>";
                echo "<p>Total: " . count($filteredData) . " dari " . count($apiData) . " data</p>";
                
                if (!empty($filteredData)) {
                    echo "<h5>Sample data kategori 3.12:</h5>";
    ...
    public function search()
    {
        ...
        try {
            $results = $this->alatPengeboranModel->searchAlatPengeboran($searchTerm, $kelompok, 10);
