...
use App\Models\RambuRambuModel;

class RambuRambu extends BaseController
{
    protected $rambuRambuModel;
    
    public function __construct()
    {
        $this->rambuRambuModel = new RambuRambuModel();
    }
    ...
    {
        $rambuRambuList = $this->getApiData();
        return view('user/barang/peralatandanmesin/ramburambu/dashboard', [
            'rambuRambuList' => $rambuRambuList
        ]);
    }

    public function kelompokRambuRambu()
    {
        $sort = $this->request->getGet('sort') ?? 'kode_barang';
        $order = $this->request->getGet('order') ?? 'asc';
        
        $allRambuRambuList = $this->rambuRambuModel->findAll();
        ...
        foreach ($kelompokList as $kelompok) {
            $data = array_filter($allRambuRambuList, function ($item) use ($kelompok) {
        ...
        return view('user/barang/peralatandanmesin/ramburambu/kelompokramburambu', [
            'sort' => $sort,
            'order' => $order,
            'allData' => $allData,
    ...
    public function kelompokDetail($kelompok)
    {
        ...
        if (!$this->rambuRambuModel->isValidKelompokRambuRambu($kelompok)) {
        ...
        $rambuRambuList = $this->rambuRambuModel->searchRambuRambu(
            $searchTerm, 
            $kelompok, 
            $perPage, 
            ($page - 1) * $perPage
        ...
        if (empty($rambuRambuList) && !empty($searchTerm)) {
            $rambuRambuList = $this->rambuRambuModel->searchRambuRambu(
                $searchTerm, 
                '', 
                $perPage, 
        ...
        if (!empty($rambuRambuList)) {
            usort($rambuRambuList, function($a, $b) use ($sort, $order) {
                $aVal = $a[$sort] ?? '';
                $bVal = $b[$sort] ?? '';
        ...
        $totalItems = !empty($searchTerm) 
            ? count($this->rambuRambuModel->searchRambuRambu($searchTerm, $kelompok, 999999, 0))
            : $this->rambuRambuModel->countByKelompok($kelompok);

        $pager = service('pager');
        $pager->setPath('user/barang/peralatandanmesin/ramburambu/kelompokramburambu/' . 
                        urlencode($kelompok));
    ...
    // Method untuk menambah rambu-rambu manual
    public function tambah()
    {
        log_message('info', '=== TAMBAH RAMBU-RAMBU METHOD DIPANGGIL ===');
        ...
        if ($isPost && (!empty($postData) || !empty($postRaw))) {
            ...
            $kategori_detail = $this->rambuRambuModel->mapKelompokToKategori($kelompok);
            ...
            try {
                $this->rambuRambuModel->skipValidation(true);
                $insertResult = $this->rambuRambuModel->insert($data);
                
                if ($insertResult) {
                    $insertId = $this->rambuRambuModel->getInsertID();
                    session()->setFlashdata('success', "Data rambu-rambu berhasil disimpan! ID: {$insertId}");
                } else {
                    $errors = $this->rambuRambuModel->errors();
                    session()->setFlashdata('error', 'Gagal menyimpan data: ' . implode(', ', $errors));
                }
                
                $this->rambuRambuModel->skipValidation(false);
    ...
    {
        log_message('info', '=== IMPORT RAMBU-RAMBU FROM API STARTED ===');
        ...
        try {
            ...
            // Kelompok yang valid untuk kategori 3.18
            $validKelompok = [
                'RAMBU-RAMBU LALU LINTAS DARAT',
                'RAMBU-RAMBU LALU LINTAS UDARA'
            ];
            ...
            foreach ($allApiData as $index => $apiItem) {
                try {
                    ...
                    // FILTER: Hanya import data dengan kelompok kategori 3.18 yang valid
                    if (!in_array($kelompok_raw, $validKelompok)) {
                        $filtered++;
                        log_message('info', "Filtered out: {$kode_barang_original} - Kelompok: '{$kelompok_raw}' (bukan kategori 3.18)");
                        continue;
                    }
                    ...
                    $cleanData = $this->rambuRambuModel->cleanImportData($apiItem);
                    ...
                    // Cek kelompok valid
                    if (!$this->rambuRambuModel->isValidKelompokRambuRambu($cleanData['kelompok'])) {
                    ...
                    $existingData = $this->rambuRambuModel->findByKodeBarang($unique_kode);
                    
                    if ($existingData) {
                        // Update existing data
                        $result = $this->rambuRambuModel->updateData($existingData['id'], $cleanData);
                        if ($result) {
                            $updated++;
                        ...
                        // Insert new data
                        $result = $this->rambuRambuModel->insertData($cleanData);
                        if ($result) {
                            $imported++;
    ...
    {
        try {
            $this->rambuRambuModel->builder()->truncate();
    ...
    public function exportRambuRambuList($kelompokSlug)
    {
        // Mapping slug ke kelompok yang sebenarnya
        $kelompokMapping = [
            'rambulalulintasdarat' => 'RAMBU-RAMBU LALU LINTAS DARAT',
            'rambulalulintasudara' => 'RAMBU-RAMBU LALU LINTAS UDARA'
        ];
        ...
        try {
            $data = $this->rambuRambuModel->getByKelompok($kelompok);
            ...
            $filename = 'rambu_rambu_' . strtolower(str_replace([' ', '/', '&', '-'], '_', $kelompok)) . '_' . date('Y-m-d_H-i-s') . '.csv';
    ...
    {
        try {
            $stats = $this->rambuRambuModel->getDashboardData();
    ...
    public function testApi()
    {
        try {
            ...
            echo "<h3>Test API Rambu-Rambu</h3>";
            ...
            if (!empty($apiData)) {
                ...
                echo "<h4>Data yang akan diimport (kategori 3.18):</h4>";
                echo "<p>Total: " . count($filteredData) . " dari " . count($apiData) . " data</p>";
                
                if (!empty($filteredData)) {
                    echo "<h5>Sample data kategori 3.18:</h5>";
    ...
    public function search()
    {
        ...
        try {
            $results = $this->rambuRambuModel->searchRambuRambu($searchTerm, $kelompok, 10);
