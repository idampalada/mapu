...
class AlatAngkutanModel extends Model
{
    ...
    protected $allowedFields = [
        ...
        'daya_mesin',
        'bhn_bakar',
        'kelompok', // ALAT ANGKUTAN DARAT BERMOTOR, ALAT ANGKUTAN DARAT TAK BERMOTOR, dll
        'sub_kelompok',
        'created_at',
        'updated_at',
    ];
    ...
    public function searchAlatAngkutan($searchTerm = '', $kelompok = '', $limit = 100, $offset = 0)
    {
        $builder = $this->builder();
        ...
        if (!empty($searchTerm)) {
            ...
            $builder->groupStart()
                ...
                ->orLike('daya_mesin', $searchTerm)
                ->orLike('bhn_bakar', $searchTerm)
                ->groupEnd();
        }
    ...
    public function validateImportData($data)
    {
        ...
        // Year validation
        if (isset($data['thn_buat']) && 
            !empty($data['thn_buat'])) {
            if (!is_numeric($data['thn_buat']) || strlen($data['thn_buat']) != 4) {
                $errors[] = 'Tahun buat harus berupa 4 digit angka';
            } else {
                $tahun = intval($data['thn_buat']);
    ...
    public function cleanImportData($data)
    {
        ...
        $cleaned['thn_buat'] = trim($data['thn_buat'] ?? '');
        $cleaned['no_mesin'] = trim($data['no_mesin'] ?? '');
        $cleaned['no_rangka'] = trim($data['no_rangka'] ?? '');
        $cleaned['no_polisi'] = trim($data['no_polisi'] ?? '');
        $cleaned['daya_mesin'] = trim($data['daya_mesin'] ?? '');
        $cleaned['bhn_bakar'] = trim($data['bhn_bakar'] ?? '');
