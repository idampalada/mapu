...
class JaringanModel extends Model
{
    ...
    // 20. Get data dari API
    // ================== API METHODS ============
public function getAllJaringanFromApi()
{
    $client = \Config\Services::curlrequest();
    $apiKey = 'c877acaa0de297a9e3b8bbdb101dd254d33a92a0444b979d599e04fdeaccdbc5';
    $apiUrl = "http://apigw.pu.go.id/v1/siman/instalasi-jaringan?api_key={$apiKey}";
    ...
    try {
        ...
        if ($response->getStatusCode() === 200) {
            ...
            $allData = $result['resource'] ?? [];
            
            // Filter hanya data jaringan
            $validKelompok = $this->getValidKelompok();
            $jaringanData = array_filter($allData, function($item) use ($validKelompok) {
                $kelompok = strtoupper($item['kelompok'] ?? '');
                return in_array($kelompok, $validKelompok);
            });
            
            return array_values($jaringanData);
        }
    } catch (\Exception $e) {
        log_message('error', 'API error in JaringanModel: ' . $e->getMessage());
    }
    
    return [];
}

// Update juga method testApiConnection
public function testApiConnection()
{
    ...
    try {
        ...
        if ($response->getStatusCode() === 200) {
            $result = json_decode($response->getBody(), true);
            $totalItems = isset($result['resource']) ? count($result['resource']) : 0;
    ...
    // 22. Validation method - Pure PHP validation untuk import data
    ...
    // 23. Validation method - Validate kelompok (untuk form input)
    public function validateKelompok($kelompok)
    {
        return $this->isValidKelompokJaringan($kelompok);
    }
    
    // 24. Helper method - Get valid kelompok list
    ...
    // 25. Helper method - Clean data untuk import
    ...
    // 26. Helper method - Safe float conversion
